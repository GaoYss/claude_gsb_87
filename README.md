...
- 销号必须同时提供验收意见（`content`）和销号日期（`closed_on`），缺项返回 HTTP 422 并逐项列明，
  且不会改动原状态、不留下流水；销号流水记录操作人、时间与验收意见，可随时回看。
- 状态推进采用「当前状态仍为已读值」的条件更新：两人同时销号同一条隐患时只有一次生效，
  另一方收到 HTTP 409 提示，不会被静默覆盖。
